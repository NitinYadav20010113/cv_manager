...
from general.tblmodels import Tblcv, Tblsearch, Tblcontractmode,Tblfundagencies,Tblpavement,Tblterrain,Tblprodetails,Tblphase, Tblsector, Tblindustry, Tblsearchmeta, Tblcvkeywords, Tblkeywords, Tbllanguage_gen, Tbledumode, Tbledulevel,Tblcvfiles,Tbladdress,Tblcountries, Tblcities, Tblstates,Tbleducation,Tblexperience,Tblfundedproject,Tblsubjects,Tbldesignation,Tbldegree, Tblinstitutes, Tbluniversities,cvm_run_history
...
@csrf_exempt
def final(file_path,old_path,new_file_path):
	try:
		global candid_id
		global file
		...
		if file.endswith('.pdf'):
			...
			try:
				...
				try:
					...
					if email_check:
						...
						if not exist:
							cvm_run_history.objects.using('table_db').create(candidate_id=candid_id,file_name=file,file_path=old_path,duplicate=1,infracon=1,completed=0)
				...
				# 	# saving the file with the prefix of candid id
				...
				# 	# file_path1 = fs.url(fullnamee)
				# 	# file_path1 =  str(BASE_DIR) + file_path1
				# 	# logdata.append('coping file to server')	
				# 	if new_file_path not in os.listdir('/media/serverceg/apps/applications/public_html/cvm/storage/app/public/'):
				# 		shutil.move(new_file_path,'/media/serverceg/apps/applications/public_html/cvm/storage/app/public/')
				# 	# shutil.copy(file_path1,str(BASE_DIR)+'/files/')														
			...
			exist=cvm_run_history.objects.using('table_db').filter(file_name=file).exists()
			if not exist:
				cvm_run_history.objects.using('table_db').create(file_name=new_file_name,file_path=new_file_path,completed=1,infracon=1)
		...
		exist=cvm_run_history.objects.using('table_db').filter(file_name=file).exists()
		if not exist:
			cvm_run_history.objects.using('table_db').create(file_name=file,status=0,infracon=1,completed=0)